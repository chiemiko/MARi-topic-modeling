...
def plot_word_freq_comparison(resume_text, df, filename):
    """
    Plotting function for the graph Word Frequency Comparison graph in the first row
    """
...
def img_to_text(image_location):
    img = Image.open('resources/' + image_location)
    text=pytesseract.image_to_string(img)
    return text

def make_plots1():
    """
    Helper function which returns an html.Div object for the first body row (drag and drop upload
    and Word Frequency Comparison graph)
    """
    return html.Div(
        style={ "padding": "10px"},
        children=[
...
def make_plots2():
    """
    Helper wrapper for the html.Div object that holds the various plot objects in the second 
    row
    """
    return html.Div(
            children=[
...
fig = plot_word_freq_comparison(example_resume, df, filename)
...
def parse_contents(contents, filename, date):
    ...
    clf =  pickle.load( open( "resources/trigram_model.pkl", "rb" ) )
    cv =  pickle.load( open( "resources/count_vectorizer.pkl", "rb" ) )
...
@app.callback(
            ...
            Output("word-freq-comparison-plot", "figure"),
            ],
...
def update_wordcloud_treemap_output(list_of_contents, list_of_names, list_of_dates):
    if list_of_contents is not None:
        ...
        fig = plot_word_freq_comparison(text, df, list_of_names)
    else:
        wordcloud, frequency_figure, treemap = plotly_wordcloud(example_resume)
        fig = plot_word_freq_comparison(example_resume, df, filename)
    return [wordcloud, frequency_figure, treemap, fig]
